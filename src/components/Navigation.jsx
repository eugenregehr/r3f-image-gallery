...
export default function Navigation({ images }) {
  ...
  const findImageIndex = (title) => images.findIndex(el => el.title === title);
  ...
  const handleNavigation = (direction) => {
    ...
    setLocation(`/${images[nextIndex].title}`);
  }
      ...
      <div onClick={() => (setEnterClicked(true), setLocation(images[0].title))} className='flex items-center justify-center bg-gray-700 opacity-75 hover:opacity-100 p-4 rounded-full h-20 w-20 cursor-pointer text-white'>
        ...
        <div className='flex items-center justify-center gap-4'>
          <div onClick={() => handleNavigation('left')} 
            className='bg-gray-700 opacity-50 hover:opacity-100 p-4 rounded-full h-16 w-16 text-black cursor-pointer rotate-180'>
            <img src="arrow.svg" alt="arrow-left" />
          </div>
          <div onClick={() => (setLocation('/'), setEnterClicked(false))} className='bg-gray-700 opacity-50 hover:opacity-100 p-4 rounded-full h-16 w-16 text-black cursor-pointer'>
          <img src="home.svg" alt="home" /> 
          </div>
          <div onClick={() => handleNavigation('right')} className='bg-gray-700 opacity-50 hover:opacity-100 p-4 rounded-full h-16 w-16 text-black cursor-pointer'>
